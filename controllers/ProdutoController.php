...
//Para modificar acesse o /views/insumos/_form.php
  ...
  if ($model->load(Yii::$app->request->post()) ) {
    ...
    public function actionLucroproduto($idProduto){
         $valorvenda = Yii::$app->db->createCommand('SELECT valorVenda FROM produto WHERE idProduto=:idProduto', ['idProduto' => $idProduto])->queryOne();

         $valorcompra = Yii::$app->db->createCommand('SELECT valorCompra FROM compraproduto WHERE idProduto=:idProduto ORDER BY idcompra DESC LIMIT 1', ['idProduto' => $idProduto])->queryOne();

         //return $valorvenda['valorVenda'];
         return $valorvenda['valorVenda'] - $valorcompra['valorCompra'];
    }
}