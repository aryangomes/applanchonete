...
$arraychange = array();
$arrayproptrue = array();
$arraypropfalse  =array();
$aaux = array();
$script = "";
$aux = array();
$auxpropfalse = array();
$arrayvalues = array();
$auxarrayvalues = array();
for ($i = 0; $i < count($permissoes); $i++) {
    for ($j = 0; $j < count(array_keys($permissoes[$i])); $j++) {

        array_push($teste, array_keys($permissoes[$i])[$j]);
    ...
    $aaux[$i] = $teste;
    $teste = [];
    $auxarrayvalues[$i] = implode(",", $aaux[$i]);
    array_push($arrayvalues, "[".$auxarrayvalues[$i]."]");
    $auxarrayvalues = [];
}
...
for ($i = 0; $i < count($aaux); $i++) {
    ...
    for ($j = 0; $j < count($aaux[$i]); $j++) {
        if ($j == 0) {
            array_push($arraychange, ("$(\"[value='" . $aaux[$i][0] . "']\")"));
        }
        array_push($arrayproptrue, ("$(\"[value='" . $aaux[$i][$j] . "']\").prop('checked', true);"));
        array_push($arraypropfalse, ("$(\"[value='" . $aaux[$i][$j] . "']\").prop('checked', false);"));


    }
    $aux[$i] = $arrayproptrue;
    $auxpropfalse[$i] = $arraypropfalse;
...
for ($i = 0; $i < count($aux); $i++) {
    $str[$i] = implode(" ", $aux[$i]);
    $strpropfalse[$i] = implode(" ", $auxpropfalse[$i]);
}
...
//aqui
       ...
       " . $arraychange[$i] . ".change(function () {
          
        if   (" . $arraychange[$i] . ".is(':checked')) {
...
//aqui
        ...
        $(\"[value='".$teste[$i][$j]."']\").change(function () {
          
        if ($(\"[value='".$teste[$i][$j]."']\").is(':checked')) {

                $(\"[value='".$teste[$i][$j]."']\").prop('checked', true);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', true);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', true);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', true);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', true);
                ...
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', false);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', false);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', false);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', false);
                $(\"[value='".$teste[$i][$j]."']\").prop('checked', false);
